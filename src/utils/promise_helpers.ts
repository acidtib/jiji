...
/**
 * Execute operations in batches with error collection
 *
 * Processes operations in sequential batches, collecting errors
 * from all batches before returning.
 *
 * @param operations - Array of async operations to execute
 * @param batchSize - Size of each batch
 * @returns Object with successful results and collected errors
 */
export async function executeBatchedWithErrorCollection<T>(
  operations: (() => Promise<T>)[],
  batchSize: number = 10,
): Promise<AggregatedResults<T>> {
  const allResults: T[] = [];
  ...
  for (let i = 0; i < operations.length; i += batchSize) {
    ...
    const batchResults = await executeWithErrorCollection(batch);
 ...
 *
 * Similar to Array.map() but for async operations with error collection.
 *
 * @param items - Array of items to process
 * @param mapper - Async function to process each item
 * @returns Object with successful results and errors
 */
export async function mapWithErrorCollection<T, R>(
  items: T[],
  mapper: (item: T, index: number) => Promise<R>,
): Promise<AggregatedResults<R>> {
  const operations = items.map((item, index) => () => mapper(item, index));
  return await executeWithErrorCollection(operations);
}

/**
 * Filter operations that can fail
 *
 * Similar to Array.filter() but for async predicates with error handling.
 * Failed predicates are treated as false.
 *
 * @param items - Array of items to filter
 * @param predicate - Async predicate function
 * @returns Filtered items and any errors from predicate evaluation
 */
export async function filterWithErrorHandling<T>(
  items: T[],
  predicate: (item: T, index: number) => Promise<boolean>,
): Promise<{ filtered: T[]; errors: Error[] }> {
  const results = await mapWithErrorCollection(items, predicate);

  const filtered: T[] = [];

  // Pair up original items with their predicate results
  for (let i = 0; i < items.length; i++) {
    // Find the result for this index (results may be sparse due to errors)
    const resultIndex = results.results.findIndex((_, resultIdx) => {
      // This is a simplified approach - in practice we'd need better tracking
      return resultIdx === i - results.errorCount;
    });

    if (resultIndex >= 0 && results.results[resultIndex]) {
